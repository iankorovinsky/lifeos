...
const webLog = grid.set(0, 2, 1, 10, contrib.log, {
  label: ' web ',
  fg: 'cyan',
  selectedFg: 'cyan',
  style: {
...
// Pipe output to logs
processes.web.stdout.on('data', (data) => {
  const lines = data
    .toString()
    .split('\n')
    .filter((line) => line.trim());
  lines.forEach((line) => webLog.log(line));
});

processes.web.stderr.on('data', (data) => {
  const lines = data
    .toString()
    .split('\n')
    .filter((line) => line.trim());
  lines.forEach((line) => webLog.log(colorize(line, 'red')));
});
